...
* **処理:**
    ...
    2.  **音声ファイルの長さ確認と分割:**
        ...
        *   **20分を超える場合:**
            ...
            *   分割時、隣接するチャンク間で1分間のオーバーラップ部分を作成します。例えば、最初のチャンクが0分から15分の場合、次のチャンクは14分から開始するようにします。これにより、文字起こし時にチャンクの切れ目での情報欠落を防ぎます。
            *   分割された各音声チャンクを一時ファイルとして保存します。
        *   **20分以下の場合:** 分割処理は行いません。
    3.  **ファイルアップロードと文字起こし:**
        *   **分割した場合:** 分割された各音声チャンクを順にGoogle Cloud (Gemini API経由) にアップロードします。アップロード後、各チャンクに対してGemini APIに文字起こしをリクエストします。各チャンクの処理完了後、アップロードした一時ファイルは削除します。
        *   **分割しない場合:** 受け取った単一の音声ファイルをGoogle Cloud (Gemini API経由) にアップロードし、文字起こしをリクエストします。処理完了後、アップロードしたファイルは削除します。
        *   文字起こしプロンプト例: 「この音声ファイルを文字起こししてください。」
    4.  **結合（分割した場合）:** 各チャンクから得られた文字起こしテキストを、オーバーラップ部分を適切に処理しながら結合し、一つの連続したテキストにします。
    5.  **要約:**
...
- 重要なポイントを箇条書きにする
- 各ポイントは簡潔に」
    6.  **Markdown出力:** 生成されたMarkdown形式の要約を、指定された出力先フォルダに保存します。ファイル名は元の音声ファイル名に基づいて生成します (例: `original_audio.m4a` -> `original_audio.md`)。
